...
export default function AppWrapper() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (profileLoading || !appDataReady) {
  ...
  }, [profileLoading, forceCloseProfileLoading, session, appDataReady]);
  ...
  useEffect(() => {
    ...
    if (!loading && !profileLoading && appDataReady) {
      // Tous les loadings sont finis ET les données de l'app sont prêtes
    ...
    } else if (!appDataReady) {
      console.log('⏳ App data loading...');
      setFadeIn(false);
      setDisplayMessage('Chargement des données...');
    }
  }, [loading, profileLoading, session, appDataReady]);

  // Si tout est chargé, afficher le contenu approprié
  if (displayContent) {
    const content = session ? <CommerceApp onDataReady={() => setAppDataReady(true)} /> : <AuthPage />;
    return (
  ...
  return session ? <CommerceApp onDataReady={() => setAppDataReady(true)} /> : <AuthPage />;
}
